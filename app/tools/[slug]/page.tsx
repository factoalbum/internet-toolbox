...
export default async function ToolPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="min-h-screen bg-[#faf9f6] text-[#171717]">
    ...
    <section className="container py-6 md:py-9 lg:py-11"><div className="mx-auto max-w-5xl">
      ...
      <section data-tool-workspace className="mt-8 overflow-hidden rounded-3xl border border-[#dedbd3] bg-white shadow-[0_10px_30px_rgba(23,23,23,.045)] md:mt-10" aria-labelledby="workspace-title"><div className="flex flex-col gap-3 border-b border-[#e8e5dd] px-5 py-4 sm:flex-row sm:items-center sm:justify-between md:px-6"><div className="min-w-0"><h2 id="workspace-title" className="text-sm font-black">Use {tool.name}</h2><p className="mt-0.5 text-xs leading-5 text-black/40">{workspaceHint}</p></div><span title={processingNote.external ? "This tool uses a service or live data outside the browser." : "Processing happens locally in your browser."} className="inline-flex w-fit shrink-0 items-center gap-1.5 rounded-full bg-[#f2f6e8] px-3 py-1.5 text-[11px] font-bold text-[#52691f]"><{processingNote.external ? "span" : "span"}><{processingNote.external ? "Globe2" : "LockKeyhole"} size={12} aria-hidden="true" /> {processingNote.label}</{processingNote.external ? "span" : "span"}></span></div><div className="min-w-0 overflow-hidden"><ToolRouter slug={slug} /></div></section>
