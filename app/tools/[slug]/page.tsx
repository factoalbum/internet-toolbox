...
import { ArrowLeft, ArrowRight, Info, LockKeyhole, Globe2 } from "lucide-react";
...
export default async function ToolPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="min-h-screen bg-[#faf9f6] text-[#171717]">
    ...
    <section className="container py-6 md:py-9 lg:py-11"><div className="mx-auto max-w-5xl">
      ...
      <section id="tool-workspace" data-tool-workspace className="mt-8 scroll-mt-24 overflow-hidden rounded-3xl border border-[#dedbd3] bg-[#fffdf8] shadow-[0_10px_30px_rgba(23,23,23,.045)] md:mt-10" aria-labelledby="workspace-title" aria-describedby="workspace-hint"><div className="flex flex-col gap-3 border-b border-[#e3dfd5] bg-[#f5f2ea] px-5 py-4 sm:flex-row sm:items-center sm:justify-between md:px-6"><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-[.14em] text-[#6d8e25]">Tool workspace</p><h2 id="workspace-title" className="mt-0.5 text-sm font-black">Use {tool.name}</h2><p id="workspace-hint" className="mt-0.5 text-xs leading-5 text-black/45">{workspaceHint}</p></div><span aria-label={processingNote.label} title={processingNote.external ? "This tool uses a service or live data outside the browser." : "Processing happens locally in your browser."} className="inline-flex w-fit max-w-full shrink-0 items-center gap-1.5 rounded-full bg-[#e9f1d8] px-3 py-1.5 text-[11px] font-bold text-[#52691f]"><ProcessingIcon size={12} aria-hidden="true" /> {processingNote.label}</span></div><div className="min-w-0 overflow-hidden"><Suspense fallback={<WorkspaceLoading />}><ToolRouter slug={slug} /></Suspense></div></section>
      ...
      {relatedTools.length > 0 && <section className="mt-10 border-t border-[#dedbd3] pt-8" aria-labelledby="related-tools"><div className="flex items-end justify-between gap-4"><div><p className="text-[11px] font-black uppercase tracking-[.15em] text-[#6d8e25]">Related</p><h2 id="related-tools" className="mt-2 text-2xl font-black tracking-[-.04em]">You might also need</h2></div><Link href="/tools" className="hidden min-h-10 items-center gap-2 rounded-lg px-2 text-xs font-black sm:inline-flex focus:outline-none focus:ring-4 focus:ring-[#c8f169]">All tools <ArrowRight size={13} aria-hidden="true" /></Link></div><div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4">{relatedTools.map((item) => <Link key={item.slug} href={`/tools/${item.slug}`} className="group min-w-0 min-h-36 rounded-2xl border border-[#e0ddd5] bg-white p-3.5 transition hover:-translate-y-0.5 hover:border-[#171717] hover:shadow-[0_8px_20px_rgba(23,23,23,.06)] focus:outline-none focus:ring-4 focus:ring-[#c8f169] sm:min-h-40 sm:p-4"><div className="flex h-full min-h-32 flex-col"><div className="flex items-start justify-between gap-2"><ToolIcon icon={item.icon} slug={item.slug} category={item.category} size={17} className="size-9 sm:size-11" /><ArrowRight size={14} className="mt-1 shrink-0 text-black/20 transition group-hover:translate-x-1 group-hover:text-black" aria-hidden="true" /></div><h3 className="mt-4 break-words text-xs font-bold leading-4 sm:text-sm sm:leading-normal">{item.name}</h3><p className="mt-1 line-clamp-2 break-words text-[10px] leading-4 text-black/45 sm:text-xs sm:leading-5">{item.description}</p><span className="mt-auto pt-2 inline-flex items-center gap-1 text-[9px] font-bold text-[#6d8e25] sm:mt-3 sm:pt-0 sm:text-xs">Open tool</span></div></Link>)}</div></section>}
    </div></section>
    <SiteFooter />
  </main>;
}
