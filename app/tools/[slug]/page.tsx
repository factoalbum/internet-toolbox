...
export function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  return params.then(({ slug }) => {
    ...
    return { title: tool.name, description: `${tool.description} Free to use.`, alternates: { canonical: toolUrl }, openGraph: { type: "website", title: `${tool.name} | Internet Toolbox`, description: tool.description, url: toolUrl }, twitter: { card: "summary", title: `${tool.name} | Internet Toolbox`, description: tool.description } };
  });
}
...
export default async function ToolPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const structuredData = [
    ...
    { "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: "Home", item: `${siteUrl}/` }, { "@type": "ListItem", position: 2, name: categoryName, item: categoryUrl }, { "@type": "ListItem", position: 3, name: tool.name, item: toolUrl }] },
  ];
  ...
  return (
    <main className="min-h-screen overflow-x-clip bg-[#f3f0e8] text-[#171717]">
      ...
      <SiteHeader />
      <section className="container py-12 md:py-16"><div className="mx-auto max-w-5xl min-w-0">
        <nav aria-label="Breadcrumb" className="mb-6 overflow-x-auto whitespace-nowrap text-sm text-black/50"><ol className="flex min-w-max items-center gap-2"><li><Link href="/" className="rounded-sm underline-offset-4 hover:text-[#171717] hover:underline">Home</Link></li><li aria-hidden="true">/</li><li><Link href={`/categories/${tool.category}`} className="rounded-sm underline-offset-4 hover:text-[#171717] hover:underline">{categoryName}</Link></li><li aria-hidden="true">/</li><li aria-current="page" className="font-semibold text-[#171717]">{tool.name}</li></ol></nav>
        <div className="flex min-w-0 items-start justify-between gap-6 border-b-2 border-[#171717] pb-8"><div className="min-w-0"><p className="font-mono text-xs font-bold uppercase tracking-[0.18em] text-black/40">Tool / {tool.category}</p><h1 className="mt-3 break-words text-4xl font-black tracking-[-0.04em] md:text-6xl">{tool.name}</h1><p className="mt-4 max-w-2xl break-words text-base leading-7 text-black/55 md:text-lg">{tool.description}</p></div><div className="hidden size-14 shrink-0 items-center justify-center rounded-xl bg-[#c8f169] md:flex"><Icon size={25} /></div></div>
        <div className="mt-8 min-w-0">
          {slug === "percentage-calculator" && <PercentageCalculator />}{slug === "age-calculator" && <AgeCalculator />}{slug === "discount-calculator" && <DiscountCalculator />}{slug === "time-converter" && <TimeConverter />}{slug === "time-zone-converter" && <TimeZoneConverter />}{slug === "emi-calculator" && <EmiCalculator />}{slug === "gst-calculator" && <GstCalculator />}{slug === "hra-calculator" && <HousingAllowanceCalculator />}{slug === "bmi-calculator" && <BmiCalculator />}{slug === "sip-calculator" && <SipCalculator />}{slug === "unit-converter" && <UnitConverter />}{slug === "date-calculator" && <DateCalculator />}{slug === "fd-calculator" && <FdCalculator />}{slug === "compound-interest-calculator" && <CompoundInterestCalculator />}{slug === "income-tax-calculator" && <IncomeTaxCalculator />}{slug === "salary-calculator" && <SalaryCalculator />}{slug === "ppf-calculator" && <PpfCalculator />}{slug === "currency-converter" && <CurrencyConverter />}{slug === "gold-silver-rate-converter" && <GoldSilverConverter />}{slug === "random-number-generator" && <RandomNumberGenerator />}{slug === "tip-calculator" && <TipCalculator />}{slug === "bill-splitter" && <BillSplitter />}{slug === "url-shortener" && <UrlShortener />}{slug === "message-writer" && <MessageWriter />}{slug === "json-formatter" && <JsonFormatter />}{slug === "uuid-generator" && <UuidGenerator />}{slug === "password-generator" && <PasswordGenerator />}{slug === "url-encoder-decoder" && <UrlEncoder />}{slug === "base64-encoder-decoder" && <Base64Tool />}{slug === "timestamp-converter" && <TimestampConverter />}{slug === "color-converter" && <ColorConverter />}{slug === "html-entity-encoder-decoder" && <HtmlEntityTool />}{slug === "regex-tester" && <RegexTester />}{slug === "word-counter" && <WordCounter />}{slug === "character-counter" && <CharacterCounter />}{slug === "case-converter" && <CaseConverter />}{slug === "text-cleaner" && <TextCleaner />}{slug === "remove-duplicate-lines" && <RemoveDuplicateLines />}{slug === "url-slug-generator" && <SlugGenerator />}{slug === "text-diff-checker" && <TextDiff />}{slug === "image-compressor" && <ImageCompressor />}{slug === "direct-video-downloader" && <DirectVideoDownloader />}
