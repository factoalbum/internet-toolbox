...
export default async function ToolPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="min-h-screen bg-[#faf9f6] text-[#171717]">
    ...
    <section className="container py-6 md:py-9 lg:py-11"><div className="mx-auto max-w-5xl">
      ...
      <header className="max-w-3xl"><div className="flex items-center gap-3"><span className="flex size-11 items-center justify-center rounded-2xl bg-[#e8f4c9] text-[#435816]" aria-hidden="true"><Icon size={22} /></span><div><p className="text-[11px] font-black uppercase tracking-[.15em] text-[#6d8e25]">{category.name}</p><p className="mt-0.5 text-xs font-medium text-black/40">Free online tool</p></div></div><h1 className="mt-5 text-4xl font-black leading-[1] tracking-[-.055em] sm:text-5xl md:text-6xl">{tool.name}</h1><p className="mt-4 max-w-2xl text-base leading-7 text-black/55 md:text-lg">{content.overview}</p></header>
      ...
      <section data-tool-workspace className="mt-8 overflow-hidden rounded-3xl border border-[#dedbd3] bg-white shadow-[0_10px_30px_rgba(23,23,23,.045)] md:mt-10" aria-labelledby="workspace-title"><div className="flex flex-col gap-3 border-b border-[#e8e5dd] px-5 py-4 sm:flex-row sm:items-center sm:justify-between md:px-6"><div className="min-w-0"><h2 id="workspace-title" className="text-sm font-black">Use {tool.name}</h2><p className="mt-0.5 text-xs leading-5 text-black/40">{workspaceHint}</p></div><span title={processingNote.external ? "This tool uses a service or live data outside the browser." : "Processing happens locally in your browser."} className="inline-flex w-fit shrink-0 items-center gap-1.5 rounded-full bg-[#f2f6e8] px-3 py-1.5 text-[11px] font-bold text-[#52691f]"><ProcessingIcon size={12} aria-hidden="true" /> {processingNote.label}</span></div><div className="min-w-0 overflow-hidden"><Suspense fallback={<WorkspaceLoading />}><ToolRouter slug={slug} /></Suspense></div></section>
      ...
      {relatedTools.length > 0 && <section className="mt-10 border-t border-[#dedbd3] pt-8" aria-labelledby="related-tools"><div className="flex items-end justify-between gap-4"><div><p className="text-[11px] font-black uppercase tracking-[.15em] text-[#6d8e25]">Related</p><h2 id="related-tools" className="mt-2 text-2xl font-black tracking-[-.04em]">You might also need</h2></div><Link href="/tools" className="hidden min-h-10 items-center gap-2 rounded-lg px-2 text-xs font-black sm:inline-flex focus:outline-none focus:ring-4 focus:ring-[#c8f169]">All tools <ArrowRight size={13} /></Link></div><div className="mt-4 grid gap-3 sm:grid-cols-3">{relatedTools.map((item) => <Link key={item.slug} href={`/tools/${item.slug}`} className="group min-h-40 rounded-2xl border border-[#e0ddd5] bg-white p-4 transition hover:-translate-y-0.5 hover:border-[#171717] hover:shadow-[0_8px_20px_rgba(23,23,23,.06)] focus:outline-none focus:ring-4 focus:ring-[#c8f169]"><span className="flex size-9 items-center justify-center rounded-xl bg-[#e8f4c9] text-[#435816]" aria-hidden="true"><item.icon size={17} /></span><h3 className="mt-4 text-sm font-black">{item.name}</h3><p className="mt-1 line-clamp-2 text-xs leading-5 text-black/45">{item.description}</p><span className="mt-3 inline-flex items-center gap-1 text-xs font-bold text-[#6d8e25]">Open tool <ArrowRight size={12} /></span></Link>)}</div></section>}
    </div></section>
    <SiteFooter />
  </main>;
}
