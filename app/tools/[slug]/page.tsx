...
export default async function ToolPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="min-h-screen bg-[#faf9f6] text-[#171717]">
    ...
    <section className="container py-6 md:py-9 lg:py-11"><div className="mx-auto max-w-5xl">
      ...
      {relatedTools.length > 0 && <section className="mt-10 border-t border-[#dedbd3] pt-8" aria-labelledby="related-tools"><div className="flex items-end justify-between gap-4"><div><p className="text-[11px] font-black uppercase tracking-[.15em] text-[#6d8e25]">Related</p><h2 id="related-tools" className="mt-2 text-2xl font-black tracking-[-.04em]">You might also need</h2></div><Link href="/tools" className="hidden min-h-10 items-center gap-2 rounded-lg px-2 text-xs font-black sm:inline-flex focus:outline-none focus:ring-4 focus:ring-[#c8f169]">All tools <ArrowRight size={13} aria-hidden="true" /></Link></div><div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4">{relatedTools.map((item) => <Link key={item.slug} href={`/tools/${item.slug}`} className="group min-w-0 aspect-square rounded-2xl border border-[#e0ddd5] bg-white p-3.5 transition hover:-translate-y-0.5 hover:border-[#171717] hover:shadow-[0_8px_20px_rgba(23,23,23,.06)] focus:outline-none focus:ring-4 focus:ring-[#c8f169] sm:aspect-auto sm:min-h-40 sm:p-4"><div className="flex h-full flex-col"><div className="flex items-start justify-between gap-2"><ToolIcon icon={item.icon} slug={item.slug} category={item.category} size={17} className="size-9 sm:size-11" /><ArrowRight size={14} className="mt-1 shrink-0 text-black/20 transition group-hover:translate-x-1 group-hover:text-black" aria-hidden="true" /></div><h3 className="mt-auto pt-3 break-words text-xs font-bold leading-4 sm:mt-4 sm:pt-0 sm:text-sm sm:leading-normal">{item.name}</h3><p className="mt-1 line-clamp-2 break-words text-[10px] leading-4 text-black/45 sm:text-xs sm:leading-5">{item.description}</p><span className="mt-2 inline-flex items-center gap-1 text-[9px] font-bold text-[#6d8e25] sm:mt-3 sm:text-xs">Open tool</span></div></Link>)}</div></section>}
    </div></section>
    <SiteFooter />
  </main>;
}
