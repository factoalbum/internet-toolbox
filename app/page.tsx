...
import { ArrowRight, Heart, Search, ShieldCheck, Users, Zap } from "lucide-react";
...
export const metadata: Metadata = {
  ...
  description: "Simple free online calculators, converters, text tools, developer utilities, file tools and document comparison tools.",
  alternates: { canonical: `${siteUrl}/` },
  openGraph: { type: "website", title: "Internet Toolbox | Simple Free Online Tools", description: "Simple free online calculators, converters, text tools, developer utilities, file tools and document comparison tools.", url: `${siteUrl}/` },
  twitter: { card: "summary_large_image", title: "Internet Toolbox | Simple Free Online Tools", description: "Simple free online calculators, converters, text tools, developer utilities, file tools and document comparison tools." },
};
...
export default function Home() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#faf9f6] text-[#101522]">
      ...
      <section className="container py-8 md:py-12" aria-labelledby="popular-heading"><div className="flex items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.13em] text-[#71809a]">Featured tools</p><h2 id="popular-heading" className="mt-1 text-2xl font-black tracking-tight md:text-3xl">Start with something useful</h2></div><Link href="/tools" className="inline-flex items-center gap-2 text-sm font-bold">All tools <ArrowRight size={15} /></Link></div><div className="mt-6 grid grid-cols-2 gap-2.5 sm:grid-cols-2 sm:gap-3 lg:grid-cols-4">{popular.map((tool) => { if (!tool) return null; return <Link key={tool.slug} href={`/tools/${tool.slug}`} className="group relative flex min-h-[154px] flex-col items-start justify-start rounded-2xl border border-[#dedbd3] bg-white p-3 pr-9 shadow-[0_3px_12px_rgba(23,23,23,.025)] transition hover:-translate-y-0.5 hover:border-[#bdb9af] hover:shadow-[0_12px_28px_rgba(23,23,23,.07)] focus:outline-none focus:ring-4 focus:ring-[#c8f169] sm:min-h-[112px] sm:flex-row sm:items-center sm:gap-4 sm:p-4 sm:pr-4"><ToolIcon icon={tool.icon} slug={tool.slug} category={tool.category} size={18} className="size-10 sm:size-12" /><span className="mt-2.5 min-w-0 sm:mt-0 sm:flex-1"><span className="block text-xs font-black leading-4 sm:text-sm">{tool.name}</span><span className="mt-1.5 block line-clamp-3 text-[11px] leading-[1.125rem] text-[#66758f] sm:mt-1 sm:line-clamp-2 sm:text-xs sm:leading-5">{tool.description}</span></span><ArrowRight size={16} className="absolute right-3 top-3.5 shrink-0 text-[#7d8799] transition group-hover:translate-x-0.5 group-hover:text-[#101522] sm:static sm:ml-auto sm:size-[17px]" aria-hidden="true" /></Link>; })}</div></section>
