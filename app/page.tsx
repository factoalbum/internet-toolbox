...
const quickTools = [
  ...
  { slug: "gold-silver-rate-converter", label: "Check gold & silver" },
];
...
const taskCards = [
  ...
  { label: "Work", title: "Get something done", text: "Clean text, shorten URLs and prepare content quickly.", href: "/categories/text", icon: FileText },
  { label: "Tech", title: "Solve a developer task", text: "Format JSON, generate UUIDs, encode data and debug values.", href: "/categories/developer", icon: Code2 },
  { label: "Files", title: "Fix a file", text: "Compress images and handle common browser-based file tasks.", href: "/categories/files", icon: FileImage },
];
  ...
  return (
    <main className="min-h-screen bg-[#f3f0e8] text-[#171717]">
      <header className="sticky top-0 z-20 border-b border-[#d8d4c9] bg-[#f3f0e8]/95 backdrop-blur">
        <div className="container flex min-h-[64px] items-center justify-between gap-6">
      ...
      <section className="container py-14 md:py-20" aria-labelledby="tasks-heading">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h2 id="tasks-heading" className="mt-2 text-3xl font-black tracking-tight md:text-4xl">Choose what you’re trying to do</h2>
          <p className="mt-3 text-sm leading-6 text-black/50">You don’t need to know the name of a calculator. Start with the task and we’ll take you there.</p>
        </div>
        <div className="mx-auto mt-8 grid max-w-5xl gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {taskCards.map(({ label, title, text, href, icon: Icon }) => <Link key={title} href={href} className="group min-h-44 border border-[#d8d4c9] bg-[#fffdf8] p-5 transition hover:-translate-y-0.5 hover:border-[#171717] hover:shadow-[5px_5px_0_#171717] focus:outline-none focus:ring-4 focus:ring-[#c8f169]">
      ...
      <section className="border-y border-[#d8d4c9] bg-[#e8e4d9]" aria-labelledby="popular-heading">
        <div className="container py-14 md:py-20">
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-black/35">Popular right now</p><h2 id="popular-heading" className="mt-2 text-3xl font-black tracking-tight md:text-4xl">Tools people come here for</h2></div>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {featured.map(tool => { const Icon = tool.icon; return <Link key={tool.slug} href={`/tools/${tool.slug}`} className="group border border-[#d8d4c9] bg-[#fffdf8] p-5 transition hover:border-[#171717] hover:bg-white focus:outline-none focus:ring-4 focus:ring-[#c8f169]">
              <div className="flex items-start justify-between gap-3"><span className="flex size-9 items-center justify-center rounded-lg bg-[#edf7d5]"><Icon size={18} aria-hidden="true" /></span><ArrowRight size={15} className="text-black/25 transition group-hover:translate-x-1 group-hover:text-black" /></div>
              <h3 className="mt-7 text-base font-bold tracking-tight">{tool.name}</h3>
      ...
      <section className="container py-14 md:py-20" aria-labelledby="categories-heading">
        <div className="grid gap-8 md:grid-cols-[.7fr_1.3fr] md:items-start">
          <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-black/35">Everything in one place</p><h2 id="categories-heading" className="mt-2 text-3xl font-black tracking-tight md:text-4xl">Browse by category</h2><p className="mt-4 max-w-sm text-sm leading-6 text-black/50">From money calculations to developer utilities, keep the workflow simple and go straight to the useful part.</p></div>
          <div className="grid gap-3 sm:grid-cols-2">
            {categories.map(category => { const Icon = category.icon; return <Link key={category.slug} href={`/categories/${category.slug}`} className="group flex gap-4 border border-[#d8d4c9] bg-[#fffdf8] p-5 transition hover:border-[#171717] hover:bg-[#c8f169] focus:outline-none focus:ring-4 focus:ring-[#171717] focus:ring-inset"><span className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-[#f3f0e8]"><Icon size={19} aria-hidden="true" /></span><span><span className="flex items-center gap-2 font-bold">{category.name}<ArrowRight size={14} className="opacity-25 transition group-hover:translate-x-1 group-hover:opacity-100" /></span><span className="mt-1 block text-sm leading-5 text-black/50">{category.description}</span></span></Link>; })}
          </div>
        </div>
      </section>
      ...
      <section className="border-t border-[#d8d4c9] bg-[#171717] text-white">
        <div className="container py-12 md:py-16">
          ...
          <div className="mt-9 grid gap-3 text-sm sm:grid-cols-3"><div className="border border-white/10 bg-white/5 p-5"><p className="font-bold">1. Tell us the task</p><p className="mt-2 text-white/45">Search by what you actually want to accomplish.</p></div><div className="border border-white/10 bg-white/5 p-5"><p className="font-bold">2. Enter the basics</p><p className="mt-2 text-white/45">Only the information needed for the calculation or conversion.</p></div><div className="border border-white/10 bg-white/5 p-5"><p className="font-bold">3. Get a clear answer</p><p className="mt-2 text-white/45">Results are presented in plain language, ready to use.</p></div></div>
        </div>
      </section>
