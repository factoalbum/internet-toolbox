...
export default function CategoriesPage() {
  return <main className="min-h-screen bg-[#faf9f6] text-[#171717]">
    ...
    <section className="container py-8 md:py-12" aria-labelledby="categories-heading"><h2 id="categories-heading" className="sr-only">Tool categories</h2><div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{categories.map((category) => { const Icon = category.icon; const count = tools.filter((tool) => tool.category === category.slug && tool.status === "live").length; return <Link key={category.slug} href={`/categories/${category.slug}`} className="group rounded-2xl border border-[#dedbd3] bg-white p-5 shadow-[0_3px_12px_rgba(23,23,23,.025)] transition hover:-translate-y-0.5 hover:border-[#171717] hover:shadow-[0_12px_28px_rgba(23,23,23,.07)] focus:outline-none focus:ring-4 focus:ring-[#c8f169]"><div className="flex items-start justify-between gap-4"><span className="flex size-12 items-center justify-center rounded-xl bg-[#e9f4cf] text-[#425515]"><Icon size={22} aria-hidden="true" /></span><ArrowRight size={17} className="mt-1 text-black/25 transition group-hover:translate-x-1 group-hover:text-black" aria-hidden="true" /></div><h3 className="mt-6 text-lg font-black">{category.name}</h3><p className="mt-2 text-sm leading-6 text-black/50">{category.description}</p><p className="mt-5 text-xs font-bold text-[#6d8e25]">{count} free tools</p></Link>; })}</div></section>
    <section className="border-y border-[#e4e1d9] bg-white" aria-labelledby="guidance-heading"><div className="container py-10 md:py-14"><div className="max-w-3xl"><p className="text-xs font-bold uppercase tracking-[.14em] text-[#6d8e25]">Choose the right category</p><h2 id="guidance-heading" className="mt-2 text-2xl font-black tracking-[-.035em] md:text-3xl">Start with the kind of task you have.</h2><p className="mt-3 text-sm leading-6 text-black/55 md:text-base">Each category groups tools by the job they help you complete, so you can choose a useful starting point without browsing the whole catalogue.</p></div><div className="mt-8 grid gap-4 md:grid-cols-2">{guidance.map(([name, text]) => <section key={name} className="rounded-2xl border border-[#e0ddd5] bg-[#faf9f6] p-5"><h3 className="text-sm font-black">{name}</h3><p className="mt-2 text-sm leading-6 text-black/55">{text}</p></section>)}</div></div></section>
