    ...
    <div className="relative" onBlur={event => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setFocused(false); }}>
      <div className="rounded-2xl border border-black/10 bg-black/[0.03] p-2">
        ...
        <div className="flex items-center gap-3 rounded-xl bg-[#fffdf8] px-4 py-3.5 text-[#171717] transition focus-within:ring-4 focus-within:ring-[#5f7429]/15">
          ...
          <input ref={inputRef} id="tool-search" value={query} onChange={event => { setQuery(event.target.value); setActiveIndex(-1); }} onFocus={() => setFocused(true)} onKeyDown={handleKeyDown} placeholder="Search a task or tool…" className="min-w-0 flex-1 bg-transparent text-base font-medium outline-none placeholder:text-black/35" autoComplete="off" role="combobox" aria-autocomplete="list" aria-expanded={showResults} aria-controls="tool-search-results" aria-activedescendant={activeIndex >= 0 ? `tool-result-${matches[activeIndex]?.slug}` : undefined} />
          {query ? <button type="button" onClick={clearSearch} className="rounded-md p-1 text-black/40 transition hover:bg-black/5 hover:text-black focus:outline-none focus:ring-2 focus:ring-[#c8f169]" aria-label="Clear search"><X size={16} /></button> : <kbd className="hidden rounded-md bg-black/5 px-2 py-1 font-mono text-[10px] text-black/40 sm:block">⌘ K</kbd>}
        </div>
      </div>

      {showResults && <div id="tool-search-results" role="listbox" aria-label={query ? "Tool search results" : "Popular tools"} className="absolute left-0 right-0 top-full z-20 mt-2 overflow-hidden border border-[#d8d4c9] bg-[#fffdf8] text-[#171717] shadow-[8px_8px_0_#171717]">
        {matches.length > 0 ? <>
