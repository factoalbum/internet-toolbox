...
export default function AllToolsBrowser() {
  ...
  return <div className="min-w-0">
    ...
    {filtered.length === 0 ? <div id="all-tools-results" className="mt-5 border border-dashed border-[#bcb8ae] bg-[#fffdf8] px-6 py-12 text-center" role="status"><h2 className="font-bold">No matching tools</h2><p className="mt-2 text-sm text-black/50">Try a broader search such as tax, loan, date, PDF or text.</p><button type="button" onClick={clearFilters} className="mt-5 min-h-11 rounded-lg bg-[#171717] px-5 text-sm font-bold text-white hover:bg-black/80 focus:outline-none focus:ring-4 focus:ring-[#c8f169]">Show all tools</button></div> : <div id="all-tools-results" className="mt-5 grid min-w-0 grid-cols-2 gap-3 sm:grid-cols-2 sm:gap-4 lg:grid-cols-3">{filtered.map((tool) => { const content = <><div className="flex items-start justify-between gap-2"><ToolIcon icon={tool.icon} slug={tool.slug} category={tool.category} size={17} className="size-9 sm:size-11" /><span className="font-mono text-[8px] font-bold uppercase tracking-[0.12em] text-[#6d8e25] sm:text-[10px] sm:tracking-[0.14em]">Live</span></div><h2 className="mt-auto pt-3 break-words text-xs font-bold leading-4 sm:mt-5 sm:pt-0 sm:text-base sm:leading-normal">{tool.name}</h2><p className="mt-1 line-clamp-2 break-words text-[10px] leading-4 text-black/50 sm:mt-2 sm:text-sm sm:leading-6">{tool.description}</p><span className="mt-2 inline-flex items-center gap-1 text-[9px] font-bold sm:mt-5 sm:text-xs">Open tool <ArrowRight size={11} className="transition group-hover:translate-x-0.5 sm:size-[13px]" /></span></>;
      return <Link key={tool.slug} href={`/tools/${tool.slug}`} className="group min-w-0 rounded-2xl border border-[#e1ded6] bg-white p-3.5 aspect-square hover:-translate-y-0.5 hover:border-[#171717] hover:shadow-[0_8px_24px_rgba(23,23,23,.07)] focus:outline-none focus:ring-4 focus:ring-[#c8f169] sm:aspect-auto sm:p-5">{content}</Link>; })}</div>}
  </div>;
}
