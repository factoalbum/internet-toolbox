...
export default function AllToolsBrowser() {
  ...
  return <div className="min-w-0">
    <div className="mb-8 grid min-w-0 gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
      <div className="flex min-w-0 items-center gap-3 rounded-xl border border-[#d8d4c9] bg-[#fffdf8] px-4 py-3 focus-within:border-[#171717] focus-within:ring-4 focus-within:ring-[#5f7429]/10">
        ...
        <input id="all-tools-search" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search tools" className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-black/35" autoComplete="off" />
      </div>
      <div className="flex min-w-0 flex-wrap gap-2" aria-label="Filter tools by category">
    ...
    {filtered.length === 0 ? <div className="border border-dashed border-[#bcb8ae] bg-[#fffdf8] px-6 py-12 text-center" role="status"><h2 className="font-bold">No matching tools</h2><p className="mt-2 text-sm text-black/50">Try a broader search such as tax, loan, date or text.</p><button type="button" onClick={clearFilters} className="mt-5 min-h-11 rounded-lg bg-[#171717] px-5 text-sm font-bold text-white hover:bg-black/80 focus:outline-none focus:ring-4 focus:ring-[#c8f169]">Clear search and filters</button></div> : <div className="grid min-w-0 gap-3 sm:grid-cols-2 lg:grid-cols-3">{filtered.map((tool) => { const Icon = tool.icon; const live = tool.status === "live"; return <Link key={tool.slug} href={`/tools/${tool.slug}`} className={`group min-w-0 border p-5 transition ${live ? "border-[#171717] bg-[#fffdf8] hover:-translate-y-1 hover:shadow-[7px_7px_0_#c8f169] focus:outline-none focus:ring-4 focus:ring-[#c8f169]" : "border-[#d8d4c9] bg-[#ebe7dc]"}`}><div className="flex items-start justify-between gap-3"><span className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-[#edf7d5]"><Icon size={19} aria-hidden="true" /></span><span className={`font-mono text-[10px] font-bold uppercase tracking-[0.14em] ${live ? "text-[#5b8b17]" : "text-black/30"}`}>{live ? "Live" : "Soon"}</span></div><h2 className="mt-6 break-words font-bold">{tool.name}</h2><p className="mt-2 break-words text-sm leading-6 text-black/50">{tool.description}</p><span className="mt-5 inline-flex text-xs font-bold uppercase tracking-[0.12em]">{live ? "Open tool" : "Preview"}</span></Link>})}</div>}
  </div>;
}
