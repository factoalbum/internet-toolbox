...
const supported = [
  ...
  ".py", ".java", ".go", ".rs", ".php", ".sql", ".graphql", ".gql", ".sh", ".bash", ".env",
  ".toml", ".ini", ".conf", ".vue", ".svelte", ".astro", ".json", ".yaml", ".yml", ".xml", ".svg", ".txt", ".csv",
];
...
function kindOf(name: string): FileKind {
  ...
  if ([".css", ".js", ".mjs", ".cjs", ".ts", ".tsx", ".jsx", ".py", ".java", ".go", ".rs", ".php", ".sql", ".graphql", ".gql", ".sh", ".bash", ".env", ".toml", ".ini", ".conf", ".vue", ".svelte", ".astro", ".yaml", ".yml", ".xml", ".svg"].includes(ext)) return "code";
  return "text";
}
...
function renderMarkdown(source: string) {
  ...
  lines.forEach((line, index) => {
    ...
    if (line.trim().startsWith("```")) {
      if (inCode) { output.push(<pre key={key} className="my-4 overflow-auto rounded-lg bg-[#171717] p-4 font-mono text-xs leading-6 text-white/85">{code.join("\n")}</pre>); code = []; }
      inCode = !inCode;
      return;
    }
    if (inCode) { code.push(line); return; }
    if (!line.trim()) { output.push(<div key={key} className="h-3" />); return; }
    if (line.startsWith("### ")) { output.push(<h3 key={key} className="mt-5 text-lg font-bold">{inlineMarkdown(line.slice(4))}</h3>); return; }
    if (line.startsWith("## ")) { output.push(<h2 key={key} className="mt-6 text-xl font-bold">{inlineMarkdown(line.slice(3))}</h2>); return; }
    if (line.startsWith("# ")) { output.push(<h1 key={key} className="mt-1 text-2xl font-black">{inlineMarkdown(line.slice(2))}</h1>); return; }
    if (/^[-*] /.test(line)) { output.push(<li key={key} className="ml-5 list-disc leading-7">{inlineMarkdown(line.slice(2))}</li>); return; }
    if (/^\d+\. /.test(line)) { output.push(<li key={key} className="ml-5 list-decimal leading-7">{inlineMarkdown(line.replace(/^\d+\. /, ""))}</li>); return; }
    if (line.startsWith("> ")) { output.push(<blockquote key={key} className="border-l-2 border-[#91c63f] pl-4 italic text-black/60">{inlineMarkdown(line.slice(2))}</blockquote>); return; }
...
export default function DeveloperFileViewer() {
  ...
  const readFile = (selected: File) => {
    ...
    if (!supported.includes(ext)) { setError(`Unsupported file type. Try ${supported.slice(0, 10).join(", ")} or another supported text file.`); return; }
    if (selected.size > 5 * 1024 * 1024) { setError("This tool supports files up to 5 MB."); return; }
    setError(""); setCopied(false); setFile(selected); setTab("preview");
  ...
  const clear = () => { setFile(null); setContent(""); setError(""); setCopied(false); if (inputRef.current) inputRef.current.value = ""; };
    ...
    <div className="space-y-5">
      {!file ? (
        <button type="button" onClick={() => inputRef.current?.click()} onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={(event) => { event.preventDefault(); setDragging(false); const dropped = event.dataTransfer.files[0]; if (dropped) readFile(dropped); }} className={`flex min-h-72 w-full flex-col items-center justify-center rounded-2xl border-2 border-dashed px-6 text-center transition ${dragging ? "border-[#6d8e25] bg-[#edf7d5]" : "border-[#d8d4c9] bg-[#faf9f6] hover:border-[#171717]"}`}>
          ...
          <span className="mt-5 text-lg font-black">Drop a developer file here</span>
          ...
          <span className="mt-4 max-w-2xl text-xs leading-5 text-black/40">README, HTML, CSS, JavaScript, TypeScript, Python, Java, Go, Rust, PHP, SQL, JSON, YAML, XML, SVG, config files and more. Up to 5 MB. Everything stays in your browser.</span>
        </button>
      ) : (
