    ...
    <div className="overflow-hidden rounded-2xl border border-[#d8d4c9] bg-[#fffdf8] shadow-[0_8px_24px_rgba(23,23,23,.045)]" aria-labelledby="discount-workspace-title">
      ...
      <div className="p-5 md:p-7">
        <div className="grid gap-4 md:grid-cols-2">
          <label className="block rounded-2xl border border-[#e2dfd7] bg-white p-4 transition focus-within:border-[#171717] focus-within:shadow-[0_0_0_4px_rgba(200,241,105,.3)]">
            ...
            <div className="mt-2 flex items-center rounded-xl border border-[#bcb8ae] bg-[#fffdf8] focus-within:border-[#171717] focus-within:ring-4 focus-within:ring-[#c8f169]">
              ...
              <input aria-label="Original price in rupees" value={price} onChange={(event) => setPrice(event.target.value)} inputMode="decimal" type="number" min="0" step="0.01" aria-invalid={hasInvalidInput} className={`${inputClass} mt-0 border-0 bg-transparent px-2.5 focus:ring-0`} />
            </div>
          </label>
          <label className="block rounded-2xl border border-[#e2dfd7] bg-white p-4 transition focus-within:border-[#171717] focus-within:shadow-[0_0_0_4px_rgba(200,241,105,.3)]">
        ...
        <div className="mt-6" aria-labelledby="common-discounts-heading">
          ...
          <div className="mt-2 flex flex-wrap gap-2" role="group" aria-label="Quick discount percentages">{discountOptions.map((option) => <button key={option} type="button" onClick={() => setDiscount(String(option))} aria-pressed={discount === String(option)} className={`min-h-11 rounded-full border px-3.5 text-sm font-semibold transition ${focusRing} ${discount === String(option) ? "border-[#171717] bg-[#171717] text-white" : "border-[#d8d4c9] bg-[#f3f0e8] hover:border-[#171717] hover:bg-white"}`}>{option}%</button>)}</div>
        </div>

        {result ? <section className="mt-7" aria-labelledby="discount-results-heading" aria-live="polite"><div className="mb-3 flex items-center justify-between gap-3"><h3 id="discount-results-heading" className="text-sm font-black">Your savings</h3><span className="text-xs font-semibold text-black/40">{discount}% off</span></div><div className="grid gap-3 sm:grid-cols-2"><div className="rounded-2xl border border-[#d8d4c9] bg-[#f3f0e8] p-5"><p className="text-xs font-bold uppercase tracking-[0.14em] text-black/45">You save</p><p className="mt-2 break-words text-3xl font-black tracking-[-0.03em]">{money.format(result.savings)}</p><p className="mt-1 text-xs text-black/40">Discount amount</p></div><div className="rounded-2xl border border-[#171717] bg-[#c8f169] p-5"><p className="text-xs font-bold uppercase tracking-[0.14em] text-black/55">Sale price</p><p className="mt-2 break-words text-3xl font-black tracking-[-0.03em]">{money.format(result.finalPrice)}</p><p className="mt-1 text-xs text-black/55">What remains after the discount</p></div></div></section> : <p className="mt-6 rounded-xl border border-[#ead7d2] bg-[#fff7f5] p-4 text-sm leading-6 text-[#7b3d31]" role="alert">Enter a price of 0 or more and a discount from 0% to 100%.</p>}
