...
const inputClass = `mt-3 min-h-12 w-full rounded-xl border border-[#bcb8ae] bg-[#fffdf8] px-4 text-base font-semibold text-[#171717] outline-none transition hover:border-black/30 focus:border-[#171717] focus:ring-4 focus:ring-[#c8f169] ${focusRing}`;
...
export default function RandomNumberGenerator() {
  ...
  const isDefault = min === String(DEFAULT_MIN) && max === String(DEFAULT_MAX) && count === String(DEFAULT_COUNT) && !unique && numbers.length === 0 && !error && !copied;
      ...
      <div className="grid gap-5 p-5 md:grid-cols-[1.05fr_.95fr] md:p-7">
        <div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="block rounded-2xl border border-[#e2dfd7] bg-white p-4 focus-within:border-[#171717] focus-within:shadow-[0_0_0_4px_rgb(200_241_105_/_30%)]"><span className="text-sm font-bold">Minimum</span><span className="mt-1 block text-xs leading-5 text-black/45">Smallest possible number.</span><input aria-label="Minimum number" aria-describedby={error ? "random-number-error" : undefined} type="number" value={min} onChange={(event) => { setMin(event.target.value); setError(""); }} className={inputClass} /></label>
            <label className="block rounded-2xl border border-[#e2dfd7] bg-white p-4 focus-within:border-[#171717] focus-within:shadow-[0_0_0_4px_rgb(200_241_105_/_30%)]"><span className="text-sm font-bold">Maximum</span><span className="mt-1 block text-xs leading-5 text-black/45">Largest possible number.</span><input aria-label="Maximum number" aria-describedby={error ? "random-number-error" : undefined} type="number" value={max} onChange={(event) => { setMax(event.target.value); setError(""); }} className={inputClass} /></label>
            <label className="block rounded-2xl border border-[#e2dfd7] bg-white p-4 sm:col-span-2 focus-within:border-[#171717] focus-within:shadow-[0_0_0_4px_rgb(200_241_105_/_30%)]"><span className="text-sm font-bold">How many numbers?</span><span className="mt-1 block text-xs leading-5 text-black/45">Generate 1–20 numbers at a time.</span><input aria-label="Number of random numbers" aria-describedby={error ? "random-number-error" : undefined} type="number" min="1" max={MAX_COUNT} step="1" value={count} onChange={(event) => { setCount(event.target.value); setError(""); }} className={inputClass} /></label>
          </div>

          <label className={`mt-4 flex min-h-12 cursor-pointer items-center gap-3 rounded-2xl border border-[#e2dfd7] bg-white px-4 py-3 text-sm font-semibold focus-within:ring-4 focus-within:ring-[#c8f169]/40 ${focusRing}`}><input type="checkbox" checked={unique} onChange={(event) => { setUnique(event.target.checked); setError(""); }} className="size-4 accent-[#171717]" />Generate unique numbers only</label>
          ...
          {error && <p id="random-number-error" className="mt-4 rounded-xl border border-[#ead7d2] bg-[#fff7f5] p-3 text-sm font-semibold leading-5 text-[#7b3d31]" role="alert">{error}</p>}
          <button type="button" onClick={generate} className={`mt-5 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#171717] px-5 text-sm font-black text-white transition hover:bg-black/85 ${focusRing}`}> <Dices size={16} aria-hidden="true" /> Generate numbers</button>
        </div>
