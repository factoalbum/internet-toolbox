      ...
      <header className="border-b border-[#d8d4c9] bg-[#f4f1e9] px-5 py-4 md:px-7">
        <div className="flex items-start justify-between gap-4">
          <div className="flex min-w-0 items-center gap-3">
            <span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-[#e8f1ff] text-[#315fba]" aria-hidden="true"><Code2 size={20} /></span>
      ...
      <div className="p-5 md:p-7">
        <div className="grid gap-5 md:grid-cols-2">
          <section className="rounded-2xl border border-[#e2dfd7] bg-white p-4 md:p-5" aria-labelledby="html-entity-input-title">
            <div><h3 id="html-entity-input-title" className="text-sm font-bold">Input</h3><p className="mt-1 text-xs leading-5 text-black/45">{mode === "encode" ? "Type HTML text with reserved characters." : "Paste entities such as &amp; or &#169;."}</p></div>
          ...
          <section className="rounded-2xl border border-[#e2dfd7] bg-[#f4f1e9] p-4 md:p-5" aria-labelledby="html-entity-result-title">
            <div className="flex items-start justify-between gap-3"><div><h3 id="html-entity-result-title" className="text-sm font-bold">Result</h3><p className="mt-1 text-xs leading-5 text-black/45">Your converted value updates as you type.</p></div>
              <button type="button" onClick={copyOutput} disabled={!output} className={`flex min-h-11 shrink-0 items-center gap-2 rounded-xl border px-3 text-xs font-bold transition focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#c8f169] disabled:cursor-not-allowed disabled:opacity-35 ${copyState === "error" ? "border-[#b45309] text-[#92400e]" : "border-[#cfcabf] bg-white hover:bg-[#fffdf8]"}`} aria-label={copyState === "success" ? "Result copied" : copyState === "error" ? "Copy result failed" : "Copy result"} aria-describedby={copyState === "error" ? "html-entity-copy-error" : undefined}>{copyState === "success" ? <Check size={15} aria-hidden="true" /> : <Copy size={15} aria-hidden="true" />}<span className="hidden sm:inline">{copyLabel}</span></button>
            </div>
        ...
        <div className="mt-6 rounded-2xl border border-dashed border-[#d8d4c9] bg-[#faf9f6] p-4"><p className="text-xs font-black uppercase tracking-[.1em] text-black/45">Good to know</p><p className="mt-2 text-sm leading-6 text-black/55">Encoding covers common reserved characters such as <code className="rounded bg-white px-1.5 py-0.5 font-mono text-xs text-black/70">&lt;</code>, <code className="rounded bg-white px-1.5 py-0.5 font-mono text-xs text-black/70">&gt;</code>, <code className="rounded bg-white px-1.5 py-0.5 font-mono text-xs text-black/70">&amp;</code> and quotes. Processing stays on this device.</p></div>
      </div>
    </div>
  );
}
