    ...
    <div className="overflow-hidden rounded-2xl border border-[#d8d4c9] bg-[#fffdf8] shadow-[0_8px_24px_rgba(23,23,23,.045)]">
      ...
      <div className="grid gap-px bg-[#e2dfd7] md:grid-cols-2">
        <section className="bg-[#fffdf8] p-5 md:p-7" aria-labelledby="timestamp-to-date-heading">
          ...
          <div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={useNow} className={`${action} bg-[#171717] text-white hover:-translate-y-0.5 hover:shadow-[0_6px_16px_rgba(23,23,23,.14)]`}><Clock3 size={15} aria-hidden="true" />Use current time</button><button type="button" onClick={() => { setTimestamp(""); setCopyError(false); }} className={`${action} border border-[#bcb8ae] bg-white text-black/65 hover:border-[#171717] hover:text-black`}>Clear</button></div>
        ...
        <section className="bg-[#fffdf8] p-5 md:p-7" aria-labelledby="date-to-timestamp-heading">
          ...
          {dateResult ? <div className="mt-6 grid gap-3" aria-live="polite" aria-atomic="true"><div className="rounded-2xl border border-[#d8d4c9] bg-[#f3f0e8] p-4"><div className="flex items-center justify-between gap-3"><p className="text-xs font-black uppercase tracking-[.12em] text-black/40">Unix seconds</p><button type="button" onClick={() => copyValue("seconds", String(dateResult.seconds))} className={`${action} min-h-9 px-2.5 text-xs text-black/50 hover:bg-white hover:text-black`} aria-label="Copy Unix seconds">{copied === "seconds" ? <Check size={14} aria-hidden="true" /> : <Copy size={14} aria-hidden="true" />}{copied === "seconds" ? "Copied" : "Copy"}</button></div><p className="mt-2 break-all font-mono text-xl font-black">{dateResult.seconds}</p></div><div className="rounded-2xl border border-[#d8d4c9] bg-[#f3f0e8] p-4"><div className="flex items-center justify-between gap-3"><p className="text-xs font-black uppercase tracking-[.12em] text-black/40">Unix milliseconds</p><button type="button" onClick={() => copyValue("milliseconds", String(dateResult.milliseconds))} className={`${action} min-h-9 px-2.5 text-xs text-black/50 hover:bg-white hover:text-black`} aria-label="Copy Unix milliseconds">{copied === "milliseconds" ? <Check size={14} aria-hidden="true" /> : <Copy size={14} aria-hidden="true" />}{copied === "milliseconds" ? "Copied" : "Copy"}</button></div><p className="mt-2 break-all font-mono text-xl font-black">{dateResult.milliseconds}</p></div></div> : <div className="mt-6 rounded-2xl border border-dashed border-[#cfcac0] bg-[#f8f6f0] p-5 text-sm leading-6 text-black/45"><p className="font-bold text-black/65">Your timestamp results will appear here.</p><p className="mt-1">Choose a date and time to generate Unix seconds and milliseconds.</p></div>}
        </section>
      </div>
