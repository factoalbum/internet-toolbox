    ...
    <div className="overflow-hidden rounded-2xl border border-[#d8d4c9] bg-[#fffdf8] shadow-[0_8px_24px_rgba(23,23,23,.045)]">
      ...
      <div className="grid gap-px bg-[#e2dfd7] md:grid-cols-2">
        <section className="bg-[#fffdf8] p-5 md:p-7" aria-labelledby="timestamp-to-date-heading">
          ...
          {timestampResult ? <div className="mt-6 space-y-3" aria-live="polite" aria-atomic="true"><div className="rounded-2xl border border-[#d8d4c9] bg-[#f3f0e8] p-4"><div className="flex items-center justify-between gap-3"><p className="text-xs font-black uppercase tracking-[.12em] text-black/40">UTC</p><button type="button" onClick={() => copyValue("UTC", timestampResult.utc)} className={`${action} min-h-9 px-2.5 text-xs text-black/50 hover:bg-white hover:text-black`} aria-label="Copy UTC result">{copied === "UTC" ? <Check size={14} aria-hidden="true" /> : <Copy size={14} aria-hidden="true" />}{copied === "UTC" ? "Copied" : "Copy"}</button></div><p className="mt-2 break-all font-mono text-sm leading-6">{timestampResult.utc}</p></div><div className="rounded-2xl border border-[#171717] bg-[#c8f169] p-4"><p className="text-xs font-black uppercase tracking-[.12em] text-black/50">Your local time</p><p className="mt-2 text-lg font-black leading-7">{timestampResult.local}</p></div></div> : !timestampInvalid && <div className="mt-6 rounded-2xl border border-dashed border-[#cfcac0] bg-[#f8f6f0] p-5 text-sm leading-6 text-black/45" role="status"><p className="font-bold text-black/65">Your date will appear here.</p><p className="mt-1">Enter a Unix timestamp to see the UTC and local date.</p></div>}
        </section>
