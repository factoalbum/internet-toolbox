    ...
    <div className="overflow-hidden rounded-2xl border border-[#d8d4c9] bg-[#fffdf8] shadow-[0_8px_24px_rgba(23,23,23,.045)]">
      ...
      <div className="p-5 md:p-7">
        ...
        <div className="mt-6" aria-live="polite">
          {result ? <div className="grid gap-3 sm:grid-cols-2"><div className="rounded-2xl border border-[#d8d4c9] bg-[#f3f0e8] p-5"><p className="text-xs font-bold uppercase tracking-[0.14em] text-black/45">Interest earned</p><p className="mt-2 break-words text-2xl font-black">₹{money(result.interest)}</p></div><div className="rounded-2xl border border-[#171717] bg-[#c8f169] p-5"><p className="text-xs font-bold uppercase tracking-[0.14em] text-black/50">Final amount</p><p className="mt-2 break-words text-3xl font-black">₹{money(result.amount)}</p></div></div> : <div className="rounded-2xl border border-dashed border-[#d8d4c9] bg-white p-5"><p className="text-sm font-bold">Enter valid values to calculate</p><p className="mt-1 text-sm text-black/50">Use a positive principal and time period with a non-negative interest rate.</p></div>}
        </div>
