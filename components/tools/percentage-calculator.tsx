  ...
  return (
    <div className="overflow-hidden border border-[#d8d4c9] bg-[#fffdf8] shadow-[6px_6px_0_#171717]">
      ...
      <div className="mx-5 mb-5 bg-[#171717] p-6 text-white md:mx-7 md:mb-7 md:p-7" aria-live="polite">
        ...
        {result ? <><p className="mt-2 break-words text-4xl font-black tracking-tight md:text-5xl">{formatNumber(result.primary)}</p><p className="mt-2 text-sm text-white/45">{mode === "of" ? `${formatNumber(Number(percentage))}% of ${formatNumber(Number(value))}` : `${formatNumber(Number(value))} ${mode === "increase" ? "increased" : "decreased"} by ${formatNumber(Number(percentage))}%`}</p>{mode !== "of" && <p className="mt-1 text-xs text-white/35">Change: {formatNumber(result.secondary)}</p>}</> : <p className="mt-2 text-lg text-white/55">Enter non-negative numbers to calculate.</p>}
      </div>
    </div>
  );
}
