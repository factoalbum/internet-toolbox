  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-[#d8d4c9] bg-[#fffdf8] shadow-[0_8px_24px_rgba(23,23,23,.045)]">
      ...
      <div className="grid gap-5 p-5 md:grid-cols-[1fr_auto_1fr] md:items-end md:p-7">
        <label className="block"><span className="text-sm font-bold">Number</span><input type="number" min="0" step="any" inputMode="decimal" value={value} onChange={(event) => setValue(event.target.value)} className="mt-2 h-14 w-full rounded-xl border border-[#bcb8ae] bg-white px-4 text-xl font-bold outline-none transition focus:border-[#171717] focus:ring-4 focus:ring-[#c8f169]" aria-label="Number" aria-invalid={value !== "" && (!Number.isFinite(Number(value)) || Number(value) < 0)} /></label>
        <div className="hidden pb-3 font-mono text-xl text-black/25 md:block" aria-hidden="true">×</div>
        <label className="block"><span className="text-sm font-bold">Percentage</span><div className="relative mt-2"><input type="number" min="0" step="any" inputMode="decimal" value={percentage} onChange={(event) => setPercentage(event.target.value)} className="h-14 w-full rounded-xl border border-[#bcb8ae] bg-white px-4 pr-12 text-xl font-bold outline-none transition focus:border-[#171717] focus:ring-4 focus:ring-[#c8f169]" aria-label="Percentage" aria-invalid={percentage !== "" && (!Number.isFinite(Number(percentage)) || Number(percentage) < 0)} /><span className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 font-bold text-black/35">%</span></div></label>
      </div>
      ...
      <div className="mx-5 mb-5 rounded-xl bg-[#171717] p-6 text-white md:mx-7 md:mb-7 md:p-7" aria-live="polite"><p className="font-mono text-xs font-bold uppercase tracking-[0.16em] text-white/40">Result</p>{result ? <><p className="mt-2 break-words text-4xl font-black tracking-tight md:text-5xl">{formatNumber(result.primary)}</p><p className="mt-2 text-sm text-white/45">{mode === "of" ? `${formatNumber(Number(percentage))}% of ${formatNumber(Number(value))}` : `${formatNumber(Number(value))} ${mode === "increase" ? "increased" : "decreased"} by ${formatNumber(Number(percentage))}%`}</p>{mode !== "of" && <p className="mt-1 text-xs text-white/35">Change: {formatNumber(result.secondary)}</p>}</> : <p className="mt-2 text-lg text-white/55">Enter non-negative numbers to calculate.</p>}</div>
    </div>
  );
}
