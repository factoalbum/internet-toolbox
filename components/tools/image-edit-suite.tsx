...
function NumberField({ label, value, onChange, min = 1, max = 40000 }: { label: string; value: number; onChange: (value: number) => void; min?: number; max?: number }) {
  return <label className="block"><span className="text-xs font-black uppercase tracking-[.1em] text-black/45">{label}</span><input type="number" min={min} max={max} value={value} onChange={(e) => onChange(Math.max(min, Math.min(max, Number(e.target.value) || min)))} className="mt-2 min-h-11 w-full rounded-lg border border-[#d8d4c9] bg-white px-3 font-mono text-sm outline-none focus:border-[#171717]" /></label>;
}
...
export default function ImageEditSuite({ variant }: { variant: Variant }) {
  ...
  async function run() {
    ...
    try {
      ...
      else if (variant === "image-format-converter") { outWidth = bitmap.width; outHeight = bitmap.height; }
  ...
  return <div className="border border-[#d8d4c9] bg-[#fffdf8] p-5 md:p-8"><div className="mx-auto flex max-w-3xl flex-col gap-5"><div className="flex items-start justify-between gap-4"><div><div className="mb-2 flex items-center gap-2 text-xs font-black uppercase tracking-[.14em] text-black/40"><ImageIcon size={15} /> File & Image Tools</div><h2 className="text-2xl font-black tracking-tight">{meta.title}</h2><p className="mt-2 text-sm leading-6 text-black/55">{meta.description}</p></div><button type="button" onClick={reset} disabled={!file && !error} className="min-h-10 shrink-0 rounded-md border border-[#d8d4c9] px-3 text-xs font-bold hover:bg-black/5 disabled:opacity-35">Reset</button></div>
    ...
    {file && variant === "image-rotate-flip" && <div className="grid gap-4 sm:grid-cols-3"><label className="block"><span className="text-xs font-black uppercase tracking-[.1em] text-black/45">Rotation</span><select value={rotation} onChange={(e) => setRotation(Number(e.target.value))} className="mt-2 min-h-11 w-full rounded-lg border border-[#d8d4c9] bg-white px-3 text-sm"><option value={0}>0°</option><option value={90}>90°</option><option value={180}>180°</option><option value={270}>270°</option></select></label><label className="flex items-center gap-2 self-end pb-2 text-sm font-semibold"><input type="checkbox" checked={flipH} onChange={(e) => setFlipH(e.target.checked)} className="size-4" /> Flip horizontal</label><label className="flex items-center gap-2 self-end pb-2 text-sm font-semibold"><input type="checkbox" checked={flipV} onChange={(e) => setFlipV(e.target.checked)} className="size-4" /> Flip vertical</label></div>}
    {file && variant === "image-format-converter" && <label className="block"><span className="text-xs font-black uppercase tracking-[.1em] text-black/45">Output format</span><select value={format} onChange={(e) => setFormat(e.target.value)} className="mt-2 min-h-11 w-full rounded-lg border border-[#d8d4c9] bg-white px-3 text-sm"><option value="image/jpeg">JPG</option><option value="image/png">PNG</option><option value="image/webp">WebP</option></select><p className="mt-2 text-xs text-black/40">Converting to JPG removes transparency. PNG keeps lossless quality.</p></label>}
    {error && <p role="alert" className="border border-[#171717] bg-[#f3f0e8] p-3 text-sm font-medium">{error}</p>}
    <button type="button" onClick={run} disabled={!file || busy} className="min-h-12 rounded-lg bg-[#c8f169] px-5 text-sm font-black text-[#171717] disabled:cursor-not-allowed disabled:opacity-45">{busy ? "Processing…" : meta.action}</button>
