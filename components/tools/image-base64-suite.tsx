...
function downloadText(text: string, name: string, type = "text/plain") {
  const url = URL.createObjectURL(new Blob([text], { type })); const anchor = document.createElement("a"); anchor.href = url; anchor.download = name; anchor.click(); URL.revokeObjectURL(url);
}
...
export default function ImageBase64Suite({ variant }: { variant: Variant }) {
  ...
  const reset = () => { setValue(""); setFileName(""); setMime("image/png"); setError(""); setCopied(false); if (previewUrl) URL.revokeObjectURL(previewUrl); setPreviewUrl(""); };
  ...
  const decode = () => {
    ...
    const match = raw.match(/^data:(image\/(?:jpeg|png|webp|gif|svg\+xml));base64,([A-Za-z0-9+/=\s]+)$/i);
    const data = match ? raw : `data:${mime};base64,${raw.replace(/\s/g, "")}`;
    ...
    if (previewUrl) URL.revokeObjectURL(previewUrl); setPreviewUrl(data); setMime((data.match(/^data:([^;]+)/i)?.[1] ?? mime));
  };
  ...
  return <div className="border border-[#d8d4c9] bg-[#fffdf8] p-5 md:p-8"><div className="mx-auto flex max-w-3xl flex-col gap-5"><div className="flex items-start justify-between gap-4"><div><div className="mb-2 flex items-center gap-2 text-xs font-black uppercase tracking-[.14em] text-black/40"><ImageIcon size={15} /> File & Image Tools</div><h2 className="text-2xl font-black tracking-tight">{variant === "image-to-base64" ? "Image to Base64" : "Base64 to Image"}</h2><p className="mt-2 text-sm leading-6 text-black/55">{variant === "image-to-base64" ? "Turn an image into a copyable Base64 data URL without uploading it anywhere." : "Turn a Base64 image string into a preview and downloadable image in your browser."}</p></div><button type="button" onClick={reset} disabled={!value && !error} className="min-h-10 shrink-0 rounded-md border border-[#d8d4c9] px-3 text-xs font-bold hover:bg-black/5 disabled:opacity-35">Reset</button></div>
    ...
    <div className="flex flex-col gap-2 sm:flex-row"><button type="button" onClick={variant === "image-to-base64" ? copy : decode} disabled={!value} className="inline-flex min-h-12 flex-1 items-center justify-center gap-2 rounded-lg bg-[#c8f169] px-5 text-sm font-black text-[#171717] disabled:cursor-not-allowed disabled:opacity-45">{variant === "image-to-base64" ? <><Copy size={16} />{copied ? "Copied" : "Copy Base64"}</> : "Decode image"}</button>{variant === "base64-to-image" && previewUrl && <button type="button" onClick={() => downloadText(value, fileName || "decoded-image.txt", "text/plain")} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-lg border border-[#171717] px-5 text-sm font-bold"><Download size={16} /> Save Base64</button>}</div>
    {previewUrl && variant === "base64-to-image" && <div className="overflow-hidden rounded-2xl border border-[#d8d4c9] bg-[#faf8f2] p-4"><div className="mb-3 flex items-center justify-between gap-3"><p className="text-xs font-black uppercase tracking-[.12em] text-black/40">Preview</p><a href={previewUrl} download={fileName || `decoded-image.${mime === "image/jpeg" ? "jpg" : mime === "image/svg+xml" ? "svg" : mime.split("/")[1] || "png"}`} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-[#171717] bg-white px-3 text-xs font-bold hover:bg-[#171717] hover:text-white"><Download size={14} /> Download image</a></div><div className="flex min-h-40 items-center justify-center overflow-auto rounded-xl border border-[#e5e1d8] bg-white p-4"><img src={previewUrl} alt="Decoded image preview" className="max-h-[420px] max-w-full object-contain" /></div></div>}
  </div></div>;
}
