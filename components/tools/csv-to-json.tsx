...
export default function CsvToJson() {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-[#d8d4c9] bg-[#fffdf8] shadow-[0_8px_24px_rgba(23,23,23,.045)]" aria-labelledby="csv-json-workspace-title">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0">
              <p className="text-xs font-black uppercase tracking-[0.14em] text-black/45">Developer utility</p>
          ...
          <button type="button" onClick={clear} disabled={!input && !error && !copied} className={`inline-flex min-h-11 shrink-0 items-center gap-2 rounded-xl border border-[#c9c5ba] bg-white px-3.5 text-sm font-semibold text-[#171717] transition hover:bg-black/[0.035] ${focusRing} disabled:cursor-not-allowed disabled:opacity-40`} aria-label="Clear CSV converter"><RotateCcw size={16} aria-hidden="true" /><span className="hidden sm:inline">Clear</span></button>
        </div>
      </header>

      <div className="grid gap-5 p-5 md:grid-cols-2 md:p-7">
        <section className="rounded-2xl border border-[#ddd9cf] bg-white p-4 md:p-5" aria-labelledby="csv-input-label">
          <div className="flex items-end justify-between gap-3">
            <div>
              <label id="csv-input-label" htmlFor="csv-json-input" className="block text-sm font-bold text-[#171717]">1. CSV input</label>
          ...
          <textarea id="csv-json-input" value={input} onChange={(event) => { setInput(event.target.value); setError(""); setCopied(false); }} className={`mt-4 min-h-72 w-full resize-y rounded-xl border bg-[#fffdf8] p-4 font-mono text-sm leading-6 text-[#171717] outline-none transition-shadow placeholder:text-black/25 focus:border-[#171717] focus:ring-4 focus:ring-[#c8f169]/40 ${parsed.error ? "border-[#c46b5c]" : "border-[#c9c5ba]"}`} placeholder={'name,email,role\nAisha,aisha@example.com,Designer\nRohan,rohan@example.com,Developer'} spellCheck={false} aria-describedby="csv-json-help" aria-invalid={!!parsed.error} />
          ...
          <label className={`mt-4 inline-flex min-h-11 cursor-pointer items-center gap-2 rounded-xl border border-[#ddd9cf] bg-[#faf9f6] px-3.5 text-sm font-semibold ${focusRing}`}><input type="checkbox" checked={pretty} onChange={(event) => setPretty(event.target.checked)} className="size-4 accent-[#171717]" />Pretty-print JSON</label>
        </section>

        <section className="rounded-2xl border border-[#ddd9cf] bg-[#f4f1e9] p-4 md:p-5" aria-labelledby="csv-output-label">
          <div className="flex items-end justify-between gap-3">
            <div>
              <label id="csv-output-label" htmlFor="csv-json-output" className="block text-sm font-bold text-[#171717]">2. JSON result</label>
          ...
          <textarea id="csv-json-output" value={parsed.error ? "" : output} readOnly className={`mt-4 min-h-72 w-full resize-y rounded-xl border bg-[#fffdf8] p-4 font-mono text-sm leading-6 text-[#171717] outline-none placeholder:text-black/25 focus-visible:ring-4 focus-visible:ring-[#c8f169]/40 ${hasResult ? "border-[#c9c5ba]" : "border-[#d8d4c9]"}`} placeholder="JSON result appears here..." aria-describedby="csv-output-status" />
      ...
      <footer className="flex flex-col gap-3 border-t border-[#d8d4c9] px-5 py-4 md:flex-row md:items-center md:justify-between md:px-7">
        ...
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={copy} disabled={!hasResult} className={`inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[#171717] bg-[#171717] px-5 text-sm font-bold text-white transition hover:bg-black/80 ${focusRing} disabled:cursor-not-allowed disabled:opacity-35`} aria-label={copied ? "JSON result copied" : "Copy JSON result"}><span aria-hidden="true">{copied ? <Check size={14} /> : <Copy size={14} />}</span>{copied ? "Copied" : "Copy JSON"}</button>
          <button type="button" onClick={download} disabled={!hasResult} className={`min-h-11 rounded-xl border border-[#171717] bg-white px-5 text-sm font-bold text-[#171717] transition hover:bg-[#c8f169] ${focusRing} disabled:cursor-not-allowed disabled:opacity-35`}>Download JSON</button>
        </div>
      </footer>
    </div>
  );
