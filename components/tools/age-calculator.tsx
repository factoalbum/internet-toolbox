...
function age(birth: Date, end: Date) {
  ...
  const milliseconds = end.getTime() - birth.getTime();
...
export default function AgeCalculator() {
  ...
  return (
    <div className="border border-[#d8d4c9] bg-[#fffdf8] p-5 md:p-8">
      <div className="grid gap-5 md:grid-cols-2">
        <label className="block">
          ...
          <input
            ...
            className="mt-2 min-h-12 w-full rounded-md border border-[#bcb8ae] bg-white px-3 text-base outline-none focus:border-[#171717]"
          />
        </label>
        <label className="block">
      ...
      {result ? (
        <div className="mt-7 space-y-4" aria-live="polite">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="border border-[#d8d4c9] p-4">
              <p className="text-xs text-black/45">Total days lived</p>
              <p className="mt-1 text-xl font-bold">{result.totalDays.toLocaleString("en-IN")}</p>
            </div>
            <div className="border border-[#d8d4c9] p-4">
              <p className="text-xs text-black/45">Date range</p>
              <p className="mt-1 text-sm font-semibold">{birth} → {end}</p>
            </div>
          </div>
      ...
      <p className="mt-6 border-t border-[#d8d4c9] pt-5 text-xs leading-5 text-black/50">Age is calculated using calendar years, months and days. Total days is the elapsed calendar-day difference between the two dates.</p>
    </div>
  );
}
