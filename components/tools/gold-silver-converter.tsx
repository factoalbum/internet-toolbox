...
export default function GoldSilverConverter() {
  ...
  async function loadRates() {
    ...
    try {
      ...
      try {
        ...
        setSourceName("Gold API and ExchangeRate-API fallback");
      } catch {
  ...
  return <div className="border border-[#d8d4c9] bg-[#fffdf8] p-5 md:p-8">
    ...
    <div className="mt-5 border border-[#171717] bg-[#c8f169] p-5" aria-live="polite"><p className="text-xs font-bold uppercase tracking-[0.12em] text-black/50">Today's estimated value</p><p className="mt-2 break-all text-3xl font-black">{loading ? "Loading" : result === null ? "Not available" : money(result)}</p>{perGram !== null && <p className="mt-2 text-sm text-black/60">{money(perGram)} per gram, {money(perGram * 10)} per 10g, {money(perGram * 1000)} per kg</p>}</div>
    ...
    <section className="mt-6 border-t border-[#d8d4c9] pt-5" aria-labelledby="metal-rate-board">
      <div className="flex items-end justify-between gap-3"><div><p className="font-mono text-[11px] font-bold uppercase tracking-[0.14em] text-black/40">Today's reference</p><h2 id="metal-rate-board" className="mt-1 text-lg font-black">Gold and silver in INR</h2></div><span className="text-xs text-black/40">per gram</span></div>
      <div className="mt-3 grid gap-2 sm:grid-cols-2">
        <div className="border border-[#d8d4c9] bg-[#f8f5ed] p-3"><p className="text-xs font-bold text-black/45">24K gold, 999</p><p className="mt-1 font-bold">{loading || !snapshot ? "Not available" : money(snapshot.gold24)}</p></div>
        <div className="border border-[#d8d4c9] bg-[#f8f5ed] p-3"><p className="text-xs font-bold text-black/45">22K gold, 916</p><p className="mt-1 font-bold">{loading || !snapshot ? "Not available" : money(snapshot.gold22)}</p></div>
        <div className="border border-[#d8d4c9] bg-[#f8f5ed] p-3"><p className="text-xs font-bold text-black/45">18K gold, 750</p><p className="mt-1 font-bold">{loading || !snapshot ? "Not available" : money(snapshot.gold18)}</p></div>
        <div className="border border-[#d8d4c9] bg-[#f8f5ed] p-3"><p className="text-xs font-bold text-black/45">Silver spot</p><p className="mt-1 font-bold">{loading || !snapshot ? "Not available" : money(snapshot.silver)}</p></div>
      </div>
    </section>
