  ...
  return (
    <header className={`${sticky ? "sticky top-0 z-40 " : ""}border-b border-[#dedbd3] bg-[#fffdf8]/95 backdrop-blur`}>
      <div className="container flex min-h-[68px] items-center gap-4">
        <Link href="/" className="flex min-w-0 shrink-0 items-center gap-2.5 rounded-md focus:outline-none focus-visible:ring-4 focus-visible:ring-[#c8f169]" aria-label="Internet Toolbox home">
        ...
        <nav className="hidden min-w-0 flex-1 items-center justify-center gap-1 lg:flex" aria-label="Tool categories">
          {categories.map((category) => (
            <Link key={category.slug} href={`/categories/${category.slug}`} className="rounded-md px-3 py-2 text-xs font-semibold text-black/55 transition hover:bg-[#f3f0e8] hover:text-[#171717] focus:outline-none focus-visible:ring-4 focus-visible:ring-[#c8f169]">
              {category.name}
            </Link>
          ))}
        </nav>
        ...
        <div className="ml-auto flex shrink-0 items-center gap-2">
          <Link href="/tools" aria-label="Search and browse all tools" className="hidden size-10 items-center justify-center rounded-md border border-[#dedbd3] bg-white text-black/55 transition hover:border-[#171717] hover:text-[#171717] focus:outline-none focus-visible:ring-4 focus-visible:ring-[#c8f169] sm:flex">
